...
filtered_loops_df = pd.read_csv('/mnt/md0/jjusuf/absloopquant/AbsLoopQuant_data/mESC_random_loops_filtered.csv', index_col=0)
...
for sample_name in ['T1','T2','C3','C4']:
    ...
    for chunk_index in np.arange(num_chunks):
        ...
        if chunk_index % 50 == 0:
            loop_quant_df.to_csv('/mnt/md0/jjusuf/absloopquant/AbsLoopQuant_data/MicroC_random_loop_quantification_scores.csv', float_format='%.6f')
    loop_quant_df.to_csv('/mnt/md0/jjusuf/absloopquant/AbsLoopQuant_data/MicroC_random_loop_quantification_scores.csv', float_format='%.6f')