# script to redistribute multiply mapped reads in Micro-C (updated Sep 2025)
...
parser.add_argument("--psfile", "-p", help="P(s) curve data with base-pair resolution, in csv format with columns 'inward', 'outward', 'tandem_entry', and 'tandem_exit'")
...
def P_s(dist, strand1, strand2, P_s_data=None):
    """
    ...
    Uses P(s) data if provided; otherwise, defaults to 1/s for s>1000.
    """
    if dist >= 1_000_000:  # probability is very low (and not relevant) for large separations
        return 1e-10
    if P_s_data is not None:
        if (strand1=='+') and (strand2=='-'):
            return P_s_data.loc[dist, 'inward']
        elif (strand1=='-') and (strand2=='+'):
            return P_s_data.loc[dist, 'outward']
        elif (strand1=='+') and (strand2=='+'):
            return P_s_data.loc[dist, 'tandem_entry']
        else:
            return P_s_data.loc[dist, 'tandem_exit']
    else:
        if dist < 1000:
            return 1/1000
        return 1/dist
    
def calculate_dist(side1, side2, pair):
    """
    Returns the distance between the two ends of a possible hit, where the hits are given as
    (chrom, pos0L, strand); pos0L is the 0-based left position of the hit. If the pair is
    trans, None is returned.
    """
    if side1[0]==side2[0]:
        pos1_0L, strand1 = side1[1:]
        pos2_0L, strand2 = side2[1:]
        if strand1 == '+':
            pos1 = pos1_0L
        else:
            pos1 = pos1_0L + pair['algn_ref_span1']
        if strand2 == '+':
            pos2 = pos2_0L
        else:
            pos2 = pos2_0L + pair['algn_ref_span2']
...
# open P(s) data
P_s_data = pd.read_csv(P_s_data_filename, index_col=0)
    ...
    if len(chroms_in_both)==0:  # read must be trans
        ...
        for c in range(num_possible_pairs):
            ...
            if side1[0]==side2[0]:  # cis contact
                dist = calculate_dist(side1, side2, pair)
                probabilities[c] = P_s(dist, pair['strand1'], pair['strand2'], P_s_data)
            else:  # trans contact
                probabilities[c] = 0
        if np.sum(probabilities) > 0:
            probabilities = probabilities / np.sum(probabilities)
        else:  # if all the probabilities are zero (can happen sometimes due to noise in the P(s) curve)
        ...
        if verbose:
            for c in range(num_possible_pairs):
                ...
                side2 = possible_pairs_side2[c]
                dist = calculate_dist(side1, side2, pair)
                print(f"{c} ({side1[0]},{side1[1]},{side1[2]}) ({side2[0]},{side2[1]},{side2[2]}) dist:{dist} prob:{probabilities[c]:.2e}")
